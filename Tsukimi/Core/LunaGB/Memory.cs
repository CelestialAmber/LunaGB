...
namespace Tsukimi.Core.LunaGB
{
	...
	public struct ObjectAttributes{
		...
		bit 7: priority
		*/
		public int cgbPalette => flags & 0b111;
		public int vramBank => (flags >> 3) & 1;
	...
	public class Memory
	{
		...
		*/
		...
		public byte[] vram = new byte[0x4000]; //0x8000-9FFF (0x2000 on GB, 0x4000 on GBC (2 banks))
		//FF00-FF7F: i/o registers, FF80-FFFE: hram, FFFF: interrupt flag (ie)
		public byte[] hram = new byte[0x7F]; //0xFF80-FFFE
		public byte[] wram = new byte[0x8000]; //0xC000-DFFF (0x2000 on GB (2 banks), 0x8000 on GBC (8 banks))
		...
		public bool doingVramDMATransfer = false;
		bool onGBC;
		...
		public byte GetByte(int address) {
			...
			if(address < 0x4000){
				...
				//D000-DFFF
				int bank = onGBC ? regs.wramBank : 1;
				return wram[bank*0x2000 + (address - 0xC000)];
				...
				//F000-FDFF
				int bank = onGBC ? regs.wramBank : 1;
				return wram[bank*0x2000 + (address - 0xE000)];
		...
		public void WriteByte(int address, byte b) {
			...
			if (address < 0x4000){
				...
				//D000-DFFF
				int bank = onGBC ? regs.wramBank : 1;
				wram[bank*0x2000 + (address - 0xC000)] = b;
			}else if(address < 0xF000){
				//mirror of wram bank slot 0 (echo ram)
				//E000-EFFF
				...
				//F000-FDFF
				int bank = onGBC ? regs.wramBank : 1;
				wram[bank*0x2000 + (address - 0xE000)] = b;
			}else if(address < 0xFEA0){
				//OAM
				//FE00-FE9F
		...
		public void SetIOReg(IORegister reg, byte val) {
			switch(reg){
				...
				int curTransferEnableVal = regs.SIO_EN;
			...
			if(onGBC){
				switch(reg){
					...
					case IORegister.HDMA5:
					regs.HDMA5 = val;
					if(!doingVramDMATransfer){
						//If VRAM DMA wasn't active and HDMA5 was written to, start a VRAM DMA transfer
						doingVramDMATransfer = true;
						//Calculate the number of bytes to transfer from the value in the HDMA5 register
						vramDMATransferBytesLeft = (regs.vramDMALength + 1) * 16;
						vramDMATransferIndex = 0;
					}else if(doingVramDMATransfer && regs.vramDMAMode == 0){
						//If VRAM DMA was active but bit 7 was set to 0, stop the VRAM DMA transfer
						doingVramDMATransfer = false;
					}
					break;
					case IORegister.RP:
					...
					break;
					case IORegister.BCPD:
					//only write to bcpd when vram can be accessed (not during mode 3)
					if(canAccessVRAM){
						regs.BCPD = val;
					}
					break;
					...
					break;
					case IORegister.OCPD:
					//only write to ocpd when vram can be accessed (not during mode 3)
					if(canAccessVRAM){
						regs.OCPD = val;
					}
					break;
					...
					case IORegister.PCM12:
					case IORegister.PCM34:
					Console.WriteLine("Warning: Trying to write to PCM12/PCM34 registers");
					break;
				}
			}
		}
		...
		int vramDMATransferBytesLeft;
		int vramDMATransferIndex;

		//Performs a step for VRAM DMA.
		public void VRAMDMAStep(){
			byte b = GetByte(regs.vramDMASource + vramDMATransferIndex);
			WriteByte(regs.vramDMADest + vramDMATransferIndex, b);
			vramDMATransferBytesLeft--;
			vramDMATransferIndex++;
			regs.vramDMALength = vramDMATransferBytesLeft/16 - 1; //Update the vram dma length variable in HDMA5
			//Handle underflow
			if(regs.vramDMALength == -1) regs.vramDMALength = 0b1111111;
			if(vramDMATransferBytesLeft == 0){
				doingVramDMATransfer = false;
			}
		}
