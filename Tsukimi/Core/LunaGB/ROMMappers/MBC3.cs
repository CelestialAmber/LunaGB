...
using Tsukimi.Core.LunaGB.RTC;

namespace Tsukimi.Core.LunaGB.ROMMappers
{
	...
	*/
    ...
    public override void SetByte(int index, byte val) {
		if(index < 0x2000){
			...
			//Only the lower 7 bits are read (apparently wrong???)
			byte newBank = val; //(byte)(val & 0b01111111);
