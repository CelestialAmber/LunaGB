	...
	public class Input
	{
		...
		public static void UpdateJOYP(){
			...
			if(memory.GetHRAMBit(4,(int)IORegister.P1) == 0){
				memory.hram[(int)IORegister.P1] = (byte)((joyp & 0xF0) | flags[0] | (flags[1] << 1) | (flags[2] << 2) | (flags[3] << 3));
			}else if(memory.GetHRAMBit(5,(int)IORegister.P1) == 0){
				//If the select buttons bit is 0, set select buttons
				memory.hram[(int)IORegister.P1] = (byte)((joyp & 0xF0) | flags[4] | (flags[5] << 1) | (flags[6] << 2) | (flags[7] << 3));
			}else{
