...
namespace LunaGB.Avalonia {
	public partial class MainWindow : Window {
		...
		void StopEmulationScreenEffect(){
			...
			for (int x = 0; x < 160; x++) {
				for (int y = 0; y < 144; y++) {
					...
					int r = (int)Math.Clamp(col.r*1.1f + noise*5f,0,255);
					int g = (int)Math.Clamp(col.g + noise*5f,0,255);
					int b = (int)Math.Clamp(col.b*1.4f + noise*5f,0,255);
		...
		void UpdateStatusBar(){
			float fps = emulator.currentFPS;
			int speedPercent = (int)Math.Round(100f*(fps/Options.frameRate));
			string text = string.Format("{0:0.00} FPS ({1}%) Frame time: {2:0.00} ms",fps,speedPercent,emulator.frameTime);
